...
public class PropertyManager {
    ...
    public Properties properties = new Properties(); // lightstone - priv to pub
    private File c;
    ...
    public String getString(String s, String s1) {
        if (!this.properties.containsKey(s)) {
            s1 = this.getOverride(s, s1); // lightstone
            this.properties.setProperty(s, s1);
            this.savePropertiesFile();
        }

        return this.getOverride(s, this.properties.getProperty(s, s1)); // lightstone
    }

    public int getInt(String s, int i) {
        try {
            return this.getOverride(s, Integer.parseInt(this.getString(s, "" + i))); // lightstone
        } catch (Exception exception) {
            i = this.getOverride(s, i); // lightstone
    ...
    public boolean getBoolean(String s, boolean flag) {
        try {
            return this.getOverride(s, Boolean.parseBoolean(this.getString(s, "" + flag))); // lightstone
        } catch (Exception exception) {
            flag = this.getOverride(s, flag); // lightstone
