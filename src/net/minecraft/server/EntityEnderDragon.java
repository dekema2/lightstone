...
public class EntityEnderDragon extends EntityComplex {
    ...
    private void b(List list) {
        for (int i = 0; i < list.size(); ++i) {
            ...
            if (entity instanceof EntityLiving) {
                // lightstone start - throw damage events when the dragon attacks
    ...
    private boolean a(AxisAlignedBB axisalignedbb) {
        ...
        // lightstone start - create a list to hold all the destroyed blocks
                ...
                for (int i2 = k; i2 <= j1; ++i2) {
                    ...
                    if (j2 != 0) {
                        if (j2 != Block.OBSIDIAN.id && j2 != Block.WHITESTONE.id && j2 != Block.BEDROCK.id) {
                            flag1 = true;
                            // lightstone start - add blocks to list rather than destroying them
        ...
        if (flag1) {
            // lightstone start - set off an EntityExplodeEvent for the dragon exploding all these blocks
    ...
    protected void ag() {
        ...
        if (!this.world.isStatic && this.r > 150 && this.r % 5 == 0) {
            i = expToDrop / 20; // lightstone - drop experience as dragon falls from sky. use experience drop from death event. This is now set in getExpReward()
        ...
        if (this.r == 200) {
            i = expToDrop - 10 * (expToDrop / 20); // lightstone - drop the remaining experience
