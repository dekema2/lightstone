...
import org.unxoft.lightstone.command.CraftRemoteConsoleCommandSender;
import org.unxoft.lightstone.CraftServer;
import org.unxoft.lightstone.LoggerOutputStream;
import org.unxoft.lightstone.scheduler.CraftScheduler;
import org.unxoft.lightstone.util.ServerShutdownThread;
...
public class MinecraftServer implements Runnable, ICommandListener, IMinecraftServer {
    ...
    // public WorldServer[] worldServer; // lightstone - removed!
    public long[] f = new long[100];
    public long[][] g;
    public ServerConfigurationManager serverConfigurationManager;
    public ConsoleCommandHandler consoleCommandHandler; // lightstone - made public
    ...
    // public EntityTracker[] tracker = new EntityTracker[3]; // lightstone - removed!
    ...
    public MinecraftServer(OptionSet options) { // lightstone - adds argument OptionSet
    ...
    private boolean init() throws UnknownHostException { // lightstone - added throws UnknownHostException
        this.consoleCommandHandler = new ConsoleCommandHandler(this);
        ThreadCommandReader threadcommandreader = new ThreadCommandReader(this);

        threadcommandreader.setDaemon(true);
        threadcommandreader.start();
        ConsoleLogManager.init(this); // lightstone
        ...
        this.propertyManager = new PropertyManager(this.options); // lightstone - CLI argument support
        ...
        } catch (Throwable ioexception) { // lightstone - IOException -> Throwable
        ...
        if (!this.onlineMode) {
            ...
            log.warning("To change this, set \"online-mode\" to \"true\" in the server.properties file."); // lightstone - type. Seriously. :D
        }

        this.serverConfigurationManager = new ServerConfigurationManager(this);
        // lightstone - removed trackers
    ...
    private void a(Convertable convertable, String s, long i) {
        ...
        // lightstone - removed world and ticktime arrays
        int j = this.propertyManager.getInt("gamemode", 0);

        j = WorldSettings.a(j);
        log.info("Default game type: " + j);

        // lightstone start (+ removed worldsettings and servernbtmanager)
        int worldCount = 3;
        ...
        for (int k = 0; k < worldCount; ++k) {
            ...
            if (k == 2) {
                // lightstone start (+ don't do this in server.properties, do it in bukkit.yml)
            ...
            if (k == 0) {
                world = new WorldServer(this, new ServerNBTManager(server.getWorldContainer(), s, true), s, dimension, settings, org.bukkit.World.Environment.getEnvironment(dimension), gen); // lightstone
            } else {
                String dim = "DIM-1";
                ...
                world = new SecondaryWorldServer(this, new ServerNBTManager(server.getWorldContainer(), name, true), name, dimension, settings, this.worlds.get(0), org.bukkit.World.Environment.getEnvironment(dimension), gen); // lightstone
            }
            ...
            world.tracker = new EntityTracker(this, world); // lightstone
    ...
    private void t() {
        ...
        this.server.enablePlugins(PluginLoadOrder.POSTWORLD); // lightstone
    }

    void saveChunks() { // lightstone - private -> default
    ...
    public void stop() { // lightstone - private -> public
        ...
        // lightstone start - multiworld is handled in saveChunks() already.
        ...
        try {
            if (this.init()) {
                ...
                for (long j = 0L; this.isRunning; Thread.sleep(1L)) {
                    ...
                    j += l;
                    i = k;
                    if (this.worlds.get(0).everyoneDeeplySleeping()) { // lightstone
                        this.w();
                        j = 0L;
                    } else {
                        while (j > 50L) {
                            MinecraftServer.currentTick = (int) (System.currentTimeMillis() / 50); // lightstone
                            j -= 50L;
                            this.w();
                        }
                    }
    ...
    private void w() {
        ...
        // lightstone start - only send timeupdates to the people in that world
        ...
        for (k = 0; k < this.worlds.size(); ++k) {
            ...
            // this.g[k][this.ticks % 100] = System.nanoTime() - l; // lightstone
        // } // lightstone
    ...
    public void b() {
        while (this.x.size() > 0) {
            ...
            // lightstone start - ServerCommand for preprocessing
            ...
            // this.consoleCommandHandler.handle(servercommand); // lightstone - Removed its now called in server.dispatchCommand
            this.server.dispatchCommand(this.console, servercommand); // lightstone
        }
    }
    ...
    public static void main(final OptionSet options) { // lightstone - replaces main(String args[])
        StatisticList.a();

        try {
            MinecraftServer minecraftserver = new MinecraftServer(options); // lightstone - pass in the options

            // lightstone - remove gui
    ...
    public EntityTracker getTracker(int i) {
        return this.getWorldServer(i).tracker; // lightstone
    }
    ...
    public String d(String s) {
        ...
        // this.consoleCommandHandler.handle(new ServerCommand(s, RemoteControlCommandListener.a)); // lightstone - removed
        this.server.dispatchCommand(this.remoteConsole, servercommand); // lightstone
        // lightstone end
        return RemoteControlCommandListener.a.b();
    }

    public boolean isDebugging() {
        return this.propertyManager.getBoolean("debug", false); // lightstone - don't hardcode
    }
