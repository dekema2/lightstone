...
public class NetLoginHandler extends NetHandler {
    ...
    private String i = Long.toString(d.nextLong(), 16); // lightstone - Security fix
    ...
    public void a(Packet1Login packet1login) {
        ...
        if (packet1login.a != 22) {
            ...
            if (!this.server.onlineMode) {
                ...
                (new ThreadLoginVerifier(this, packet1login, this.server.server)).start(); // lightstone
            }
        }
    }
    ...
    public void b(Packet1Login packet1login) {
        ...
        if (entityplayer != null) {
            ...
            // entityplayer.a((World) this.server.a(entityplayer.dimension)); // lightstone - set by Entity
            entityplayer.itemInWorldManager.a((WorldServer) entityplayer.world);
            // lightstone - add world and location to 'logged in' message.
            ...
            WorldServer worldserver = (WorldServer) entityplayer.world; // lightstone
            ChunkCoordinates chunkcoordinates = worldserver.getSpawn();

            entityplayer.itemInWorldManager.b(worldserver.getWorldData().getGameType());
            NetServerHandler netserverhandler = new NetServerHandler(this.server, this.networkManager, entityplayer);

            // lightstone start -- Don't send a higher than 60 MaxPlayer size, otherwise the PlayerInfo window won't render correctly.
            ...
            // this.server.serverConfigurationManager.sendAll(new Packet3Chat("\u00A7e" + entityplayer.name + " joined the game."));  // lightstone - message moved to join event
            this.server.serverConfigurationManager.c(entityplayer);
            netserverhandler.a(entityplayer.locX, entityplayer.locY, entityplayer.locZ, entityplayer.yaw, entityplayer.pitch);
            this.server.networkListenThread.a(netserverhandler);
            netserverhandler.sendPacket(new Packet4UpdateTime(entityplayer.getPlayerTime())); // lightstone - add support for player specific time
    ...
    public void a(Packet254GetInfo packet254getinfo) {
        if (this.networkManager.f() == null) return; // lightstone - fix NPE when a client queries a server that is unable to handle it.
        try {
            // lightstone start
