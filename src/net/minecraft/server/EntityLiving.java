...
// lightstone start
import org.unxoft.lightstone.TrigMath;
...
public abstract class EntityLiving extends Entity {
    ...
    public float aD;
    public float aE;
    public EntityHuman aF = null; // lightstone - prot to pub - "killer"
    protected int aG = 0;
    public int aH = 0;
    public int aI = 0;
    ...
    protected double aP;
    float aQ = 0.0F;
    public int lastDamage = 0; // lightstone - protected -> public
    protected int aS = 0;
    protected float aT;
    protected float aU;
    protected float aV;
    ...
    private Entity e;
    protected int aZ = 0;
    public int expToDrop = 0; // lightstone added
    public int maxAirTicks = 300; // lightstone added
    ...
    public void af() {
        this.am = this.an;
        super.af();
        // MethodProfiler.a("mobBaseTick"); // lightstone -- not in production code
        if (this.random.nextInt(1000) < this.a++) {
            this.a = -this.h();
            this.ae();
        }

        // lightstone start - don't inline the damage, perform it with an event
        ...
        if (this.isAlive() && this.a(Material.WATER) && !this.f() && !this.effects.containsKey(Integer.valueOf(MobEffectList.WATER_BREATHING.id))) {
            ...
            if (this.getAirTicks() != 300) { // lightstone -- only set if needed to work around a datawatcher inefficiency
    ...
    protected void ag() {
        ++this.deathTicks;
        if (this.deathTicks >= 20 && !this.dead) { // lightstone - (this.deathTicks == 20) -> (this.deathTicks >= 20 && !this.dead).
            int i;

            // lightstone start - update getExpReward() above if the removed if() changes!
            i = expToDrop;
            while (i > 0) {
    ...
    // lightstone start - delegate so we can handle providing a reason for health being regained
    public void d(int i) {
    ...
    public void d() {
        ...
        // MethodProfiler.a("ai"); // lightstone -- not in production code
        if (this.L()) {
            this.aW = false;
            this.aT = 0.0F;
            this.aU = 0.0F;
        ...
        this.ak = f;
        // MethodProfiler.a("push"); // lightstone -- not in production code
