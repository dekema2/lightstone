...
public class TileEntityFurnace extends TileEntity implements IInventory {
    ...
    public void l_() {
        ...
        // lightstone start
        int currentTick = (int) (System.currentTimeMillis() / 50); // lightstone
        int elapsedTicks = currentTick - this.lastTick;
        this.lastTick = currentTick;

        // lightstone - moved from below
        ...
        if (this.burnTime > 0) {
            this.burnTime -= elapsedTicks; // lightstone
        }

        if (!this.world.isStatic) {
            // lightstone start - handle multiple elapsed ticks
            if (this.burnTime <= 0 && this.canBurn() && this.items[1] != null) { // lightstone - == to <=
    ...
    private boolean canBurn() {
        if (this.items[0] == null) {
            ...
            // lightstone - consider resultant count instead of current count
