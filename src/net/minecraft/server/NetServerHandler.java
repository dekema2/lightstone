...
import org.unxoft.lightstone.ChunkCompressionThread;
import org.bukkit.Location;
import org.bukkit.command.CommandException;
import org.unxoft.lightstone.CraftWorld;
import org.unxoft.lightstone.block.CraftBlock;
import org.unxoft.lightstone.entity.CraftPlayer;
import org.unxoft.lightstone.event.CraftEventFactory;
import org.unxoft.lightstone.CraftServer;
import org.unxoft.lightstone.TextWrapper;
...
public class NetServerHandler extends NetHandler implements ICommandListener {
    ...
    public EntityPlayer player; // lightstone - private -> public
    private int f;
    private int g;
    private boolean h;
    private int i;
    ...
    public void a(Packet10Flying packet10flying) {
        ...
        if (!this.player.j) {
            ...
            if (this.checkMovement && !this.player.dead) {
                ...
                if (d8 > 100.0D && this.checkMovement) { // lightstone - Added this.checkMovement condition to solve this check being triggered by teleports
                ...
                this.player.onGround = packet10flying.g;
                this.minecraftServer.serverConfigurationManager.d(this.player);
                if (this.player.itemInWorldManager.b()) return; // lightstone - fixed fall distance accumulating while being in Creative mode.
    ...
    public void a(double d0, double d1, double d2, float f, float f1) {
        // lightstone start - Delegate to teleport(Location)
        Player player = this.getPlayer();
    ...
    public void a(Packet14BlockDig packet14blockdig) {
        if (this.player.dead) return; // lightstone
        ...
        if (packet14blockdig.e == 4) {
            ...
            boolean flag = worldserver.weirdIsOpCache = worldserver.dimension != 0 || this.minecraftServer.serverConfigurationManager.isOp(this.player.name); // lightstone
            boolean flag1 = false;
    ...
    public void a(Packet15Place packet15place) {
        ...
        // lightstone - if rightclick decremented the item, always send the update packet.
        // this is not here for lightstone's own functionality; rather it is to fix
        ...
        if (packet15place.face == 255) {
            ...
            // lightstone - notch decrements the counter by 1 in the above method with food,
            ...
            // lightstone start - Check if we can actually do something over this large a distance
        ...
        if (itemstack == null || itemstack.l() == 0) {
            ...
            this.player.h = false;
            // lightstone | TODO CHECK IF NEEDED -- new if structure might not need 'always'. Kept it in for now, but may be able to remove in future
    ...
    public void a(String s, Object[] aobject) {
        if (this.disconnected) return; // lightstone - rarely it would send a disconnect line twice

        a.info(this.player.name + " lost connection: " + s);
        // lightstone start - we need to handle custom quit messages
    ...
    private void handleCommand(String s) {
        ...
        /* lightstone start - No longer neaded as we have already handled it in server.dispatchCommand above.
    ...
    public void a(Packet18ArmAnimation packet18armanimation) {
        ...
        if (packet18armanimation.b == 1) {
            // lightstone start - raytrace to look for 'rogue armswings'
            float f = 1.0F;
    ...
    public void a(Packet7UseEntity packet7useentity) {
        ...
        if (entity != null && this.player.g(entity) && this.player.i(entity) < 36.0D) {
            ItemStack itemInHand = this.player.inventory.getItemInHand(); // lightstone
            if (packet7useentity.c == 0) {
                ...
                // lightstone start - update the client if the item is an infinite one
