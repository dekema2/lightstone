...
import org.unxoft.lightstone.map.RenderData;
// lightstone end
...
public class WorldMapHumanTracker {
    ...
    public byte[] a(ItemStack itemstack) {
        ...
        RenderData render = this.d.mapView.render((CraftPlayer) trackee.getBukkitEntity()); // lightstone

        if (--this.f < 0) {
            this.f = 4;
            byte[] abyte = new byte[render.cursors.size() * 3 + 1]; // lightstone
        ...
        for (int k = 0; k < 10; ++k) {
            ...
            if (this.b[i] >= 0) {
                ...
                for (int i1 = 0; i1 < abyte1.length - 3; ++i1) {
                    abyte1[i1 + 3] = render.buffer[(i1 + l) * 128 + i]; // lightstone
                }
